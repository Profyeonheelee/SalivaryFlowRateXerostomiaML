# -*- coding: utf-8 -*-
"""
Created on Wed May 27 07:09:22 2026
...
plt.rcParams['figure.dpi'] = 150 # 주피터 화면 미리보기용 해상도
plt.rcParams['pdf.fonttype'] = 42 # 출판사 요구사항: 폰트를 벡터 그대로 인식하게 함
...
# ==========================================
# 6. Supplementary Table S2 자동 생성 (모델 간 쌍대 비교)
# ==========================================
supp_s2_results = []
comparisons = [
    ('Model 1', 'Model 2'),
...
# ==========================================
# 7. 🌟 저널 권고사항 반영: 각 독립 패널별 "절대 깨지지 않는" 3가지 포맷 동시 저장 
# ==========================================
print("\n🎨 시각화 및 부트스트랩(신뢰구간) 계산 시작...")

file_suffixes = {'UFR-defined hyposalivation': 'Figure_3_UFR', 'SFR-defined hyposalivation': 'Figure_3_SFR'}
title_names = {
    'UFR-defined hyposalivation': 'Unstimulated Salivary Hypofunction (UFR < 0.1 mL/min)',
    'SFR-defined hyposalivation': 'Stimulated Salivary Hypofunction (SFR < 0.7 mL/min)'
}
...
for out_key in outcomes.keys():
    ...
    ax.set_title(title_names[out_key], fontsize=14, fontweight='bold', pad=15)
    ...
    # 🌟 3가지 무결점 포맷으로 각각 2장의 독립된 파일 분리 저장 (PNG, PDF, TIFF)
    base_filename = f'./{file_suffixes[out_key]}_Final'
    
    plt.savefig(base_filename + '.png', dpi=600, bbox_inches='tight')
    plt.savefig(base_filename + '.tiff', dpi=600, format='tiff', bbox_inches='tight')
    plt.savefig(base_filename + '.pdf', format='pdf', bbox_inches='tight') 
    
    plt.show() 
    plt.close(fig)
    print(f"  🎉 저장 완료: {base_filename} (.png, .pdf, .tiff)")